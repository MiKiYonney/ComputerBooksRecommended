...
namespace ComputerBooksRecommended.Data
{
    /// <summary>
    ...
    public abstract class SampleDataCommon : ComputerBooksRecommended.Common.BindableBase
    {
    ...
    public sealed class SampleDataSource
    {
        ...
        public async Task GetLocalFiles()
        {
           ...
           // var Folders = await Windows.ApplicationModel.Package.Current.InstalledLocation.GetFolderAsync(Path).GetResults().GetFoldersAsync();
            ...
            {
                String groupName = folder.Name.Split(new char[] { '_' })[1];
                var group = new SampleDataGroup(folder.Name, groupName, "", "", "");
                ...
                foreach (var file in files)
                {
                    ...
                    {
                        string filename = file.Name.Split(new char[] { '.' })[0].Split(new char[] { '-' })[1];
                        string titlename = file.Name.Split(new char[] { '-' })[0];
                        if (filename.Contains(indexStr))
                        {
                            group.SetImage("/" + Path + "/" + folder.Name + "/" + file.Name);
                        }
                        else
                        {
                            var pictureItem = new SampleDataItem(filename, "Top"+titlename, filename,
                ...
                foreach (var file in files)
                {
                    if (file.FileType.Equals(".txt"))
                    {
                        ...
                        {
                            var item = group.Items.First<SampleDataItem>(i => i.Subtitle.Split(new char[] { '.' })[0] == filename);
